...
const EnhancedAutonomyPanel = () => {
  ...
  const updateAutonomyLevel = async (level) => {
    ...
    try {
      ...
      toast.success(
        <div>
          <strong>Autonomy Level Updated</strong>
  ...
  return (
    <div className=\"glass rounded-sm p-6\" data-testid=\"enhanced-autonomy-panel\">
        ...
        <div className=\"relative h-4 bg-black/30 rounded-full overflow-hidden mb-2\">
          ...
          <div className=\"absolute inset-0 flex justify-between px-1\">
            {autonomyLevels.map((level) => (\n              <div
                key={level.level}
                className=\"w-1 h-full bg-gray-700\"\n                style={{ marginLeft: level.value === 0 ? 0 : '20%' }}
              ></div>
            ))}
          </div>
        </div>
      ...
      {previousLevel !== currentLevel && !updating && (
        <div className=\"p-3 bg-cyan-500/10 border border-cyan-500/30 rounded mb-4\" data-testid=\"transition-indicator\">
          <div className=\"flex items-center gap-2 text-sm text-cyan-400\">
            <AlertTriangle className=\"w-4 h-4\" />
            <span className=\"code-font\">
              Level changed: {previousLevel} → {currentLevel}
            </span>
          </div>
        </div>
      )}
...
export default EnhancedAutonomyPanel;
